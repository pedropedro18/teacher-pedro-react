...
const db = require('../db'); // ajusta ao teu ficheiro de conexão MySQL
const verificarToken = require('../middleware/verificarToken'); // o mesmo que já usas em submissoes.js
...
router.post('/', verificarToken, async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      'INSERT INTO materiais (titulo, descricao, link_pdf, nivel) VALUES (?, ?, ?, ?)',
...
// DELETE /api/materiais/:id — admin remove material
router.delete('/:id', verificarToken, async (req, res) => {
  try {
    await db.query('DELETE FROM materiais WHERE id = ?', [req.params.id]);
